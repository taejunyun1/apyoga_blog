...
import { hasMedicalClaim } from "./content-safety"
...
export function reviewText(_text: string): ReviewOutput {
  const legacyPatterns = ["치료합니다", "완치", "교정됩니다"]
  const medicalClaims: string[] = []
  const medicalSentences = _text
    .split(/[.!?。\n]+/)
    .map((sentence) => sentence.trim())
    .filter((sentence) => sentence && hasMedicalClaim(sentence))
  for (const sentence of medicalSentences) {
    const matchedLegacy = legacyPatterns.find((pattern) => sentence.includes(pattern))
    const claim = matchedLegacy ?? sentence
    if (!medicalClaims.includes(claim)) medicalClaims.push(claim)
  }
  const phrases = _text
    .split(/[.!?。\n]+/)
