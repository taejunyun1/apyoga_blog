...
7. 네이버 본문이 500자 미만이거나 결과가 부적합하면 보강 지시로 OpenAI를 한 번만 재호출한다. 두 채널 모두 네트워크 오류, 429, 5xx, schema 불일치에는 한 번만 재시도한다.
8. 재시도 후에도 실패하거나 인증 오류처럼 재시도할 수 없는 오류가 발생하면 해당 채널을 로컬 provider로 생성한다.
...
- 네트워크 오류, 429, 5xx, schema 불일치와 500자 미만 네이버 결과는 한 번만 재시도한다. OpenAI 인증 오류와 기타 비재시도 4xx에는 재시도하지 않는다. 무제한 재시도는 하지 않는다.
